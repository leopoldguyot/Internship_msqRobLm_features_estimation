...
- This subset ensure that the model is not rank deficient.
]
...
- Does not check for rank deficiency in the model, just return the estimable coefficients.
...
- Using a subsetting when the model matrix is already constructed makes unable to check the completeness of the groups burried in the intercept. Therefore when a reference group has no values, the model is rank deficient.

- Within the algorithm there is a check to see if the model is rank deficient, if this is the case nothing is returned for the concerned feature => This is what cause the issue of low number of features fitted.
]
...
- Both msqrobLm and msqrobLm fixed models returns a fitError model, which means that the models are not
estimable
...
print(paste("The scplainer model returns a NP ratio that is equal to",
            result_scp_nfit))
print("Some coefficients are still estimated due to the use of a ridge regression.")
print(paste("The msqrob model returns a",
            result_msqrobLm_nfit,
            "model."))
print(paste("The msqrob model with the fix returns a"
            , result_msqrobLm_fix_nfit,
            "model."))
print(paste("The limma model returns",
            result_limma_nfit,
            "coefficients."))
```
]]
...
NP ratio larger than 1 indicating that the model is usable in the scplainer
...
print(paste("The msqrobLm model returns a",
            result_msqrobLm_nfit_msqrob,
            "model."))
print(paste("The msqrobLm model with the fix returns a"
...
bench_res_modelled <- read.csv("data_output/benchmark_results_modelled.csv")
bench_res_modelled$secondes<- bench_res_modelled$time/(1e9)

ggplotly(ggplot(bench_res_modelled, aes(x = Method, y = secondes, color = Method)) +
           geom_boxplot(),
         width = 800, height = 400
         )
```
...
.bigger1-3[
- We should integrate a new check in the msqrob2 code to check that an interaction variable cannot disappear due to the subsetting. This would cause issue in the interpretabilty of the model.
- I am currently working on the integration of lm.fit in scp to allow a faster model fitting. 
- We also suggest that it would be interesting to have tools in msqrob2 to investigate how the model fitting went. (e.g. investigate the number and type of errors of the linear models)
]
