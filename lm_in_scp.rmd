...
- As we can see with the 3D plot, the coefficients returned by msqrobLm, msqrobLm fixed and limma are almost identical.
]
]
.panel[
...
.panel-name[Success Results Comments]
.bigger1-3[
- As we can expect, only msqrobLm return a fitError object. This show us that the fix enable the estimation of some unestimable peptides.
]
]
.panel[
.panel-name[Success Results Code]
...
- We can see that when we compare the coefficients of the msqrobLm fix and limma, the coefficients are almost identical. This shows us that the estimation of the new msqrobLm is reliable.
]
.panel[
.panel-name[Coefficients Results Code]
.scrollable[
