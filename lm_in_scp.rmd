...
Model for a paetide that is fitted for each method
...
- PS: msqrobLm is not able to fit models for all the peptides, so we will use
msqrob2::msqrob that use msqrobLm under the hood to fit the models.
]
]
.panel[
.panel-name[Code]
...
msqrob_nfit <- rowData(msqrob2::msqrob(non_fitted,
                                   ~ 1 + Channel + Set + Mock,
                                   robust = FALSE,
                                   ridge = FALSE))$msqrobModels[[1]]
...
scp_fit <- scp::scpModelWorkflow(fitted,
                            formula = ~ 1 + Channel + Set + Mock)
msqrob_fit <- rowData(msqrob2::msqrob(fitted,
                                   ~ 1 + Channel + Set + Mock,
                                   robust = FALSE,
                                   ridge = FALSE))$msqrobModels[[1]]
...
scp_nfit_msqrob <- scp::scpModelWorkflow(non_fitted_msqrob,
                                          formula = ~ 1 + Channel + Set + Mock)
msqrob_nfit_msqrob <- rowData(msqrob2::msqrob(non_fitted_msqrob,
                                   ~ 1 + Channel + Set + Mock,
                                   robust = FALSE,
                                   ridge = FALSE))$msqrobModels[[1]]
...
return a normal output even with poor data. This can cause a problem since these
coefficients are not reliable, and will be taken into account in the
...
result_msqrob_nfit <- msqrob_nfit@type
...
.panel-name[Success Results Comments]
.bigger1-5[
As intended, each method fitted a model for the peptide. scplainer return a 
NP ration larger than 1 indicating that the model is usable in the scplainer
workflow. And both the normal and fixed msqrob return a model. limma also return
a model.
]
]
.panel[
...
result_msqrob_fit <- msqrob_fit@type
...
```{r coef_fit}
coef_msqrob_fit <- getCoef(msqrob_fit)
...
summary_coef_fit <- full_join(full_join(
      data.frame("parameter" = names(coef_msqrob_fit),
                    "coefficient_msqrob" = coef_msqrob_fit),
...
fig <- plot_ly(summary_coef_fit, x = ~coefficient_msqrob,
...
result_msqrob_nfit_msqrob <- msqrob_nfit_msqrob@type
...
```{r coef_nfit_msqrob}
# sum_to_dummy <- function(x){
#     # x: numeric vector of coefficients
#     gamma <- x[-1]
#     beta <- gamma - sum(gamma)/length(x)
#     beta <- c(0, beta)
#     names(beta) <- names(x)
#     return(beta)
# }
# # Convert sum coding to dummy coding for scplainer
# model_scp_nfit_msqrob <- scp:::scpModelFitList(scp_nfit_msqrob)[[1]]
# beta <- sum_to_dummy(model_scp_nfit_msqrob@coefficients)

coef_msqrob_nfit_msqrob <- getCoef(msqrob_nfit_msqrob) # NA
...
subset_msqrob <- msqrob2::msqrob(subset,
                                ~ 1 + Channel + Set + Mock,
                                robust = FALSE,
                                ridge = FALSE)
subset_msqrobLm_fix <- msqrob2_fix(assay(subset),
                                  ~ 1 + Channel + Set + Mock,
                                  colData(subset),
                                  robust = FALSE)
