...
contrast_msqrob <- hypothesisTest(subset_msqrob, contrast = L_msqrob, acceptDifferentReference = T)
...
```{r}
pe <- setdiff(to_test_fix, to_test_msqrob)[5]
limma_coef_pe <- subset_limma$coefficients[pe,]

df <- data.frame(y = assay(subset)[pe,], colData(subset)) %>% 
    dplyr::filter(!is.na(y)) %>%
    droplevels()
ref_class <- levels(df$Set)[1]
new_ref_class <- tail(levels(df$Set),1)
test_class <- levels(df$Set)[2]

df <- df %>% 
    dplyr::filter(Set %in% c(ref_class, test_class, new_ref_class))
ggplot(df,aes(x = Set, y = y))+geom_point()


limma_coef_pe_test_class <- limma_coef_pe[paste0("Set", test_class)]
df_mean <- df %>% 
    group_by(Set) %>% 
    summarise(mean = mean(y))
df_mean$mean[2]-df_mean$mean[3] == limma_coef_pe_test_class
```

find how to discover that the reference dropped

```{r}
dropped <- setdiff(to_test_fix, to_test_msqrob)
not_dropped <- to_test_msqrob
dropped_matrix <- lapply(dropped, function(p){
    subsubset <- subset[p,!is.na(assay(subset)[p,]), drop = FALSE]
    model.matrix(~ -1 + Set, colData(subsubset))
})

res_dropped <- lapply(dropped_matrix, function(x) sum(x[,"SeteAL00219"]) == 0)

not_dropped_matrix <- lapply(not_dropped, function(p){
    subsubset <- subset[p,!is.na(assay(subset)[p,]), drop = FALSE]
    model.matrix(~ -1 +Set, colData(subsubset))
})

res_not_dropped <- lapply(not_dropped_matrix, function(x) sum(x[,"SeteAL00219"]) == 0)

summary(unlist(res_dropped))
summary(unlist(res_not_dropped))
```
